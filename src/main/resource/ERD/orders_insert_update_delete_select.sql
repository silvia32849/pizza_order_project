...
-- 2) 주문 상세 삽입
insert into order_item(oi_no,oi_qty,order_no,product_no) values(order_item_oi_no_SEQ.nextval,1,3,2);
insert into order_item(oi_no,oi_qty,order_no,product_no) values(order_item_oi_no_SEQ.nextval,2,3,3);
--2. update
...
-- 3). 회원 한사람의 주문내역 전체삭제
delete from order_item where order_no in(select order_no from orders where user_id='test');
...
select * from order_item where order_no=3;
